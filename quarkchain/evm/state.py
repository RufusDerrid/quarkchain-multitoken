...
from quarkchain.evm.token import Token
...
class State:
    ...
    def transfer_value(self, from_addr, to_addr, value, token_id):
        assert value >= 0
        if self.get_balance(from_addr, token_id) >= value:
            self.delta_balance(from_addr, -value, token_id)
            self.delta_balance(to_addr, value, token_id)
            return True
        return False

    def deduct_value(self, from_addr, value, token_id):
        assert value >= 0
        if self.get_balance(from_addr, token_id) >= value:
            self.delta_balance(from_addr, -value, token_id)
            return True
    ...
    def del_account(self, address, token_id):
        self.set_balance(address, 0, token_id)
    ...
    def from_snapshot(cls, snapshot_data, env, executing_on_head=False):
        ...
        if "alloc" in snapshot_data:
            for addr, data in snapshot_data["alloc"].items():
                ...
                assert len(addr) == 20
                if 'wei' in data:
                    state.set_balance(addr, parse_as_int(data['wei'], 0)) #TODO get token_id from snapshot
                if 'balance' in data:
                    state.set_balance(addr, parse_as_int(data['balance'], 0))  #TODO get token_id from snapshot
                if 'code' in data:
